...
export function Header(props) {
    ...
    const compQ = useQuery(getTagsRequest, {
        variables: { start: nullthrows(search.split(" ").pop()) },
    });
    ...
    return (
        <header className={css.header}>
            <div className={css.flex}>
                <BarsIcon
                    onClick={() =>
                        setBar(bar == Bars.NAV ? Bars.NONE : Bars.NAV)
                    }
                />
                ...
                {is_anon ? (
                    <>
                        <a
                            onClick={() =>
                                setBar(
                                    bar == Bars.LOGIN ? Bars.NONE : Bars.LOGIN,
                                )
                            }
                        >
                            Log In
                    ...
                    <>
                        <a
                            onClick={() =>
                                setBar(bar == Bars.USER ? Bars.NONE : Bars.USER)
                            }
                        >
                            {meQ?.data?.me?.name}{" "}
                            {(meQ?.data?.me?.private_message_unread_count ||
                                0) > 0 && <BellIcon />}
                        </a>
                    </>
                )}
            ...
            {bar == Bars.USER && (
                <div className={css.flex}>
                    ...
                    <span>
                        Messages ({meQ?.data?.me?.private_message_unread_count})
                    </span>
                    <span className={css.fill}></span>
